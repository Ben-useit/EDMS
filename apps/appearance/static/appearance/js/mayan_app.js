...
class MayanApp {
  ...
  static async setupDropdownDirectionChange() {
    $('body').on('shown.bs.dropdown', '.dropdown', function () {
      ...
      const sizeUpwards =
        elemenMenuOffset.top -
        $elementMenu.height() -
        $elementMenuButton.height();

      const spaceDownwards =
        $(window).scrollTop() + $(window).height() - sizeDownwards;
      const spaceUpwards = sizeUpwards - $(window).scrollTop();

      if (
        (spaceUpwards >= 0 || spaceUpwards > spaceDownwards) &&
        spaceDownwards < 0
      ) {
  ...
  async setupMultiItemActions() {
    ...
    $('body').on(
      'click',
      '#multi-item-actions .navigation-btn-dropdown',
      function (event) {
        const $this = $(this);
        ...
        $('.check-all-slave:checked').each(function (index, value) {
          ...
          idList.push(value.name.split('_')[1]);
        });
  ...
  async setupFormElementContentCopy() {
    ...
    app.partialNavigationApp.$ajaxContent.on('updated', function (event) {
      const $selector = $(`.${cssClassSelector}`).not(
        `.${cssClassSelectorAttached}`
      );
    ...
    app.partialNavigationApp.$ajaxContent.on(
      ...
      function (event) {
        ...
        const $source = $this
          .parent()
          .parent()
          .children(`.${cssClassSelectorAttached}`);

        navigator.clipboard.writeText($source.val()).then(
          function () {
          ...
          function () {
            updateTooltip(
              $this,
              gettext('Failed. Check clipboard permissions.')
            );
          }
        );
      }
    ...
    $('body').on('click', '.check-all-slave', function (e) {
      ...
      if (e.shiftKey) {
        ...
        $checkBoxes
          ...
          .prop('checked', app.lastChecked.checked)
          .trigger('change');
      }
      app.lastChecked = this;
    });
      ...
      if (!targetSelection) {
        ...
        if (
          ...
          if (event.shiftKey) {
            ...
            $checkBoxes
              .slice(Math.min(start, end), Math.max(start, end) + 1)
              .prop('checked', app.lastChecked.prop('checked'))
              .trigger('change');
          }
          app.lastChecked = $checkbox;
  ...
  async setupResizePersist() {
    ...
    app.partialNavigationApp.$ajaxContent.on(
      'click',
      '.appearance-btn-resize-reset',
      function (event) {
        const $this = $(this);
        const $source = $this
          .parent()
          .siblings('.appearance-resize-persist')
          .first();
